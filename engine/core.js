...
(function (root, factory) {
  ...
  function spectral(prices, opt) {
    ...
    let p = Stats.clean(prices).filter(v => v > 0);
    if (p.length < MIN) return { ok: false, reason: `عيّنة ${p.length} جلسة — التحليل الطيفي يتطلب ${MIN}+` };

    /* ⏱️ سقف معلن لطول العيّنة الطيفية.
       الدوريّة تُحسب بدورية فورييه المباشرة، وكلفتها O(N²/6): على 246 جلسة
       0.9 مللي، وعلى 1250 جلسة 18.8 مللي — أي 21 ضعفاً مقابل 5 أضعاف
       البيانات. وبعد تثبيت تاريخ التحليل على خمس سنوات صار مسح 250 سهماً
       يكلّف 80 ثانية بدل 4.

       والسقف ليس تنازلاً عن الدقة: النطاق المفحوص يتوقف عند ثلث العيّنة،
       فعيّنة 1250 جلسة تفحص دورات تصل إلى 416 جلسة — أي سنة وثمانية أشهر،
       وهي دورة لا تُتداول أصلاً ولا تكفي خمس سنوات لتكرارها ثلاث مرات
       بمعنى مفيد. أما قدرة اختبار فيشر على النطاق القابل للتداول (6–250
       جلسة) فتُشبع قبل ذلك بكثير.

       ولأنه رقم ثابت معلن هنا — لا نافذة يختارها المستخدم بزرّ — يبقى
       المخرَج حتمياً: نفس السهم يعطي نفس الطيف مهما حُمّل من تاريخ. */
    const maxBars = opt.maxBars == null ? 750 : opt.maxBars;
    if (maxBars > 0 && p.length > maxBars) p = p.slice(p.length - maxBars);

    /* ① الدلالة تُختبر على العوائد اللوغاريتمية (أقرب للاستقرارية)

       ⚠️ حدّ معلن لاختبار فيشر: يفترض عوائد بيضاء. قياسنا على 200 سلسلة
       لكل حالة أعطى نسبة رفض كاذب: مشي عشوائي 3.0–5.0٪ (أي عند المستوى
       الاسمي تماماً)، عوائد AR(1) φ=0.3 → 6.5–8.0٪، عوائد MA(1) θ=−0.4
       → 7.0–13.5٪ وتزداد بطول العيّنة. أي أن سهماً عوائده مرتبطة ذاتياً
       (زخم أو ارتداد قصير) يُنتج «دورة دالة» بنسبة تفوق المعلنة إلى ثلاثة
       أضعاف. لهذا لا يُكتفى بدلالة فيشر وحدها: KSATiming.cycleStability
       يشترط أن تكون الدورة نفسها عبر نوافذ متداخلة قبل نشر أي تاريخ. */
    const rets = [];
